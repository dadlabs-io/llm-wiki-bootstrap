...
**The At a glance block (2026-09-15).** When `task.md` starts with `## At a glance` (the `/task-list` block, above NOW), keep it current through the script, never by hand: `python {{WIKI_SCRIPTS_DIR}}/wiki-tasks.py done <N>` for each task finished this session, `add "<task>" --owner <section>` for work the session took on or left for the user, `set <N> --status <s> --next "<text>"` where a task moved. Never `remove`: after marking tasks done, ask the user whether to remove them, and remove only on their word (`remove <N> --confirmed`). A `task.md` without the block is left without one.
...
**Create it if it is missing (2026-09-13).** Nothing seeds this file — not `wiki-init`, not `new-wiki` — so the first `/wrap-up` of a project writes it: a one-line title (`# Active context — <project>`), a one-line note that each persona edits only its own section, then one section for the current persona (`## <persona>` with Status / Recent / Next). A missing file is never a reason to skip (c); a cold start's resume read (the SessionStart hook, or the Resuming fallback in CLAUDE.md) reads this file first, and before this rule a project could reach its second session with `handoff.md` present and no `active-context.md`. The resume read tolerates all three dashboards being absent (the hook lists whichever exist and prints nothing when none do; the CLAUDE.md fallback reads each **if present**, falls back to the newest journal under `sessions/<persona>/`, else treats the project as new), so this rule closes the gap from the writing side only.
