...
2. **Working-memory dashboards** (always, Step 0.5) — refreshes the mutable resume pointer: `sessions/<persona>/handoff.md` (overwrite), `sessions/<persona>/task.md` (NOW/QUEUE, update in place), and your section of `sessions/active-context.md` (created on the first wrap-up if the project has none — nothing else seeds it). These are the memory-bank replacement — a wrap-up never leaves them stale, and after the first wrap-up all three exist for the Resuming step in CLAUDE.md to read on startup.
