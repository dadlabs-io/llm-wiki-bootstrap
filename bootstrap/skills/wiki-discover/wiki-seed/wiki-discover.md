...
**Input / Output:** Reads `_config/feeds.md`, which lists your trusted authors, YouTube channels, GitHub repos, academic queries, and vendor blogs along with each feed's covered date range. Produces a dated checklist grouping candidates by HIGH and MEDIUM relevance with a one-line reason each. When the topic has `_inbox/intake-*/` folders, candidates are classified by bucket and each bucket gets its own checklist at `_inbox/intake-<bucket>/<date>-discovery.md` for that bucket's owner to review; topics without intake folders get one combined checklist at `_inbox/discovered/<date>-discovery.md`. The run's stats and its three-section decisions log — Queued, Skipped, Deferred — recording why every URL was or wasn't taken go to `_inbox/reports/discovery-<date>.md`. Approved candidates are queued to `_inbox/pending/`, and each searched feed's last-queried date is advanced so the next run picks up where this one stopped.
