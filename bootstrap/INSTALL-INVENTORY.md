...
| `wiki-lint-mechanical.py` | Deterministic lint: broken links, orphans, frontmatter loadability, body checks (warn-only backlog), installed-skill drift, qmd index coverage; a tier-`self` entry without `raw_path` counts as self-authored, not missing (2026-09-15); `--cycle-id` + `--run-folder` write the cycle's `lint-mechanical.json` / `.md` (2026-09-24); a page declaring `standalone: "<reason>"` is listed apart, not as an orphan (2026-09-24) |
