...
| `wiki-qmd-query.py` | Runs qmd's full search (`qmd query`) for every caller — session, `/wiki-discover`, parallel ingest workers — holding one of two GPU slots (OS file locks; a third caller waits), with a 120 s timeout that kills the process tree, GPU-full back-off and retry then exit 75 (never a keyword fallback), `--preflight` (the CUDA check) and `--stats` (per-call wait/run log at `~/.cache/wiki-qmd/searches.jsonl`); `--notebook <name>` scopes a search to one notebook's collection (resolved through the registry; ingest, update and discover always pass it), `--all-notebooks` searches every one; depth is `-k` (results, default 10) and `-C` (candidates the reranker scores, default 40), also `$WIKI_QMD_K` / `$WIKI_QMD_C`. Added 2026-09-13 when the user retired the workers-use-`qmd search` rule |
