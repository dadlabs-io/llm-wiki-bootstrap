...
- **Concept table hub page** (from the nanzhipro Karpathy-bootstrap skill, ingested 2026-09-08) — one row per durable concept with a working definition, a **status** (`high confidence` / `single-source` / `tentative` / `needs sources` / `contradicted`) and a **maintenance note** ("what the next ingest or lint should watch for"), sitting between `_INDEX` and the entries; a lint check for concept-table drift (page without a row, row without a page, status no longer matching). Our glossary + concept-gaps pages hold definitions and gaps but not per-concept status or maintenance duty. Needs a design pass: which skill maintains the rows (`wiki-update` on ingest, `wrap-up` on promote) and how it relates to the glossary.
- **`framework-version` on scaffolded files + a re-scaffold diff** (from the 2026-09-04 research-to-framework process handoff) — the six framework-contract docs carry `framework-version: 3`; scaffolded files (`.claude/`, root `README.md`, `CLAUDE.md`, folder READMEs, `_config/`) and skills do not, so the Phase 2 "which projects predate the current framework" question is answered by hand. `install.py` has the TODO at the overwrite site. A `--check` mode that diffs a project's framework-managed files against the current templates (the analogue of the agent-builder's `lint-artifact.py --check-install`) makes Phase 2 mechanical.
- **Register this repo's own notebook as a qmd collection** — `project-notebooks/notebooks/llm-wiki-bootstrap/wiki/` is not in `qmd ls`, so `/wiki-search` inside this repo returns nothing (found by the lint's new coverage section, 2026-09-08). One `qmd collection add` + `qmd embed`; also worth having `new-wiki.py` do it at scaffold time when qmd is present.
